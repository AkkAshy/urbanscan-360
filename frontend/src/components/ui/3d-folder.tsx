...
interface AnimatedFolderProps {
  ...
  className?: string;
  gradient?: string;
  /** Внешний обработчик клика на превью — если задан, встроенный лайтбокс отключается */
  onProjectClick?: (index: number) => void;
}

const AnimatedFolder: React.FC<AnimatedFolderProps> = ({ title, description, projects, className, gradient, onProjectClick }) => {
  ...
  const handleProjectClick = (project: Project, index: number) => {
    // Если есть внешний обработчик — используем его вместо встроенного лайтбокса
    if (onProjectClick) {
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-sm font-medium text-muted-foreground transition-all duration-500" style={{ opacity: isHovered ? 0.8 : 1 }}>
            {projects.length} {projects.length === 1 ? 'фото' : projects.length < 5 ? 'фото' : 'фото'}
          </p>
        </div>
        {description && (
          <p className="absolute bottom-4 left-4 right-4 text-center text-xs text-muted-foreground/60 truncate transition-all duration-500" style={{ opacity: isHovered ? 0 : 1, transform: isHovered ? "translateY(10px)" : "translateY(0)" }}>
            {description}
          </p>
        )}
      </div>
